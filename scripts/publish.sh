#!/usr/bin/env bash
# Publish the demiurg workspace to crates.io, in dependency order.
#
...
# bug.) The `demiurg` editor binary lives in `demiurg-app`, so it goes
# last, after every library it links.
#
...
#
# Anti-rot gate (mirrors roxlap's publish.sh): the ORDER list below
...
echo "publishing demiurg $version: ${ORDER[*]}"
